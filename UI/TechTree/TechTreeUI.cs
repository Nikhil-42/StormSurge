...
public partial class TechTreeUI : Control {
    ...
    private void DrawConnectionLines() {
        Control linesNode = techTreeContent.GetNodeOrNull<Control>("LinesLayer");
        if (linesNode == null) {
        ...
        foreach (TechNodeButton btn in nameToButton.Values) {
            foreach (string parentName in btn.BoundNode.Parents) {
                if (!nameToButton.TryGetValue(parentName, out TechNodeButton parentBtn))
                    continue;

                Color modulate = btn.BoundNode.Blocked && parentBtn.BoundNode.Blocked
    ...
    private void OnNodeHovered(string nodeName, Vector2 position) {
        ...
        TechNode<GlobalVars> node = tree.GetNode(nodeName);
        if (node == null) return;
