...
public partial class Storm : Node3D
{
	...
	{
		// The direction indicator handles right-click drag input (this is for testing with a static spawn)
		if (@event is InputEventKey keyEvent && keyEvent.Pressed && keyEvent.Keycode == Key.Space)
		{
			SpawnStormAt(0, 0, Vector2.Right);
		}
	}

	// Signal handlers for the direction indicator
	private void OnDirectionSet(Vector2 startLatLon, Vector2 direction)
	{
		var region = GameManager.Instance.GetRegion(_globe.GetRegionID(startLatLon));
		if (region == null || region.Health <= 0.0)
		{
			SpawnStormAt(startLatLon.X, startLatLon.Y, direction);
		}
		else if (GameManager.Instance.PrintDebug)
		{
			GD.Print("[Storm] Cannot spawn storm in a living region!");
		}
	}
	...
	private void SpawnStormAt(float lat, float lon, Vector2 direction = default)
	{
		...
		float stormRange = 40.0f * (weatherVars.range / 100.0f);
	...
	public override void _Process(double delta)
	{
		...
		foreach (var storm in storms)
		{
			...
			if (strength > 0.0f)
			{
				// Reduce strength based on distance traveled (storms weaken over distance)
				float rangeDepletion = totalDistance > stormRange ? 0.1f : 0.0f;
				strength -= (0.05f + rangeDepletion) * (float)delta; // Base deplete rate + range depletion
				if (strength < 0.0f)
					strength = 0.0f;
			...
			if (regionID >= 0) 
			{
				...
				// Only generate Solar if region is still alive (health > 0.0)
				if (regionAlive)
				{
	...
	{
		double baseSolarRate = damage * 0.2; 
		...
		double finalSolar = 0.025 * (baseSolarRate + strengthBonus + techTreeBonus + populationBonus) * regionMultiplier;
